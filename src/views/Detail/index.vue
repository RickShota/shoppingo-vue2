    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <zoom :img-url-list="skuImageList"></zoom>
          <!-- 小图列表 -->
          <ImageList :img-url-list="skuImageList"></ImageList>
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="c1 in spuSaleAttrList" :key="c1.id">
                ...
                <dd changepirce="0"
                    v-for="c2 in c1.spuSaleAttrValueList" :key="c2.id"
                    :class="{'active':c2.isChecked==1}"
                    @click="onActive(c2,c1.spuSaleAttrValueList)"
                >{{ c2.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" :value="num" @input="changeNum">
...
<script>
import Zoom from "@/views/Detail/Zoom/Zoom";
import ImageList from "@/views/Detail/ImageList/ImageList";
...
export default {
  ...
  computed: {
    ...
    skuImageList() {
      // 防止服务器数据没有回来，赋值一个空数组就避免undefined报错
